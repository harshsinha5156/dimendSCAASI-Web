    ...
    <section className="w-full bg-white mt-16 py-8">
      ...
      <div className="hidden md:flex justify-center gap-10 flex-wrap">
        {diamonds.map((diamond, idx) => (
          <div
            key={idx}
            className="relative group w-20 h-20 lg:w-24 lg:h-24"
          >
            <img
              src={diamond.img}
              alt={diamond.name}
          ...
          {diamonds.map((diamond, idx) => (
            <SwiperSlide key={idx}>
              <div className="relative group w-20 h-20 mx-auto">
                ...
                <div
                  className="absolute inset-0 flex items-center justify-center 
                  text-gray-800 text-xs font-medium opacity-0 group-hover:opacity-100 
                  transition duration-300 text-center px-1"
                >
                  {diamond.name}
                </div>
              </div>
